...
verify_deployment() {
    ...
    if gsutil ls "gs://$BUCKET_NAME/gemini-lr-tagimg-v2.4.0.zip" &> /dev/null; then
...
test_deployment() {
    ...
    if command -v curl &> /dev/null; then
        ...
        # Test download link
        local download_url="https://storage.googleapis.com/$BUCKET_NAME/gemini-lr-tagimg-v2.4.0.zip"
