...
verify_deployment() {
    ...
    if gsutil ls "gs://$BUCKET_NAME/gemini-lr-tagimg-v3.0.0.zip" &> /dev/null; then
...
test_deployment() {
    ...
    if command -v curl &> /dev/null; then
        ...
        # Test download link
        local download_url="https://storage.googleapis.com/$BUCKET_NAME/gemini-lr-tagimg-v3.0.0.zip"
