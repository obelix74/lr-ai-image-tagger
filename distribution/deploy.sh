...
verify_deployment() {
    ...
    if gsutil ls "gs://$BUCKET_NAME/ai-image-tagger-v2.3.1.zip" &> /dev/null; then
...
test_deployment() {
    ...
    if command -v curl &> /dev/null; then
        ...
        # Test download link
        local download_url="https://storage.googleapis.com/$BUCKET_NAME/ai-image-tagger-v2.3.1.zip"
